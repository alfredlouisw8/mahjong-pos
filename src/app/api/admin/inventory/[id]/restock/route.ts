...
export async function POST(request: NextRequest, ctx: DynContext<"/api/admin/inventory/[id]/restock">) {
  ...
  await supabase.from("inventory_movements").insert({ inventory_item_id: id, action: "restock", quantity, notes: notes ?? null })
